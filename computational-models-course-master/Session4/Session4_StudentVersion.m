...
% to do for your report: count for how many participants in each group are
% best fitted with a model with two learning rates than with one and also
% sum up the AIC difference scores for each group
...
% QA.4 
effectSizeStats = mes([],[],'hedgesg','isDep',1,'nBoot',10000) % look at doc mes to see what the inputs mean

% If you decide that you should do non-parametric tests, complete the code
% below:
[pNonParam,hNonParam] = signrank([],[])
effectSizeStatsNonParam = mes([],[],'U1','isDep',1,'nBoot',10000)
...
effectSizeStats = mes(avgRating(Controls),avgRating(Stress),'hedgesg')
...
display('Between subject t-test comparing the change in learning rates between the two groups');
[h,p,ci,stats] = ttest2([],[])
effectSizeStats = mes([],[],'hedgesg')
...
display('Correlation between learning rate difference and average rating (across all subjects)');
[rho, pval] = corr([],[])
